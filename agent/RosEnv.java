...
import ros.msgs.curiosity_mars_rover_description_msgs.Move3;
...
//					System.out.println();
		...
		bridge.subscribe(SubscriptionRequestMsg.generate("/monitor_error")
				.setType("std_msgs/String")
//				.setThrottleRate(1)
				.setQueueLength(4),
			new RosListenDelegate() {
...
//					System.out.println();
//					if (msg.data.equals("done")) {
						Literal error = new Literal("error");
						//movement_completed.addTerm(new StringTermImpl(msg.data));
						addPercept(error);
//					}
//					else {
...
//					System.out.println();
	...
	public Unifier executeAction(String agName, Action act) throws AILexception {
		...
		if (actionname.equals("hello_ros")) {
			...
			NumberTerm speedreq = (NumberTerm) act.getTerm(2);
			control_wheels(modereq.getString(),(int) distancereq.solve(), (float) speedreq.solve());
	...
	public void control_wheels(String modereq, int distancereq, float speedreq) {
		Publisher wheels_control = new Publisher("/gwendolen_curiosity_wheels", "curiosity_mars_rover_description/Move3", bridge);
		wheels_control.publish(new Move3(modereq, distancereq, speedreq));
	}
